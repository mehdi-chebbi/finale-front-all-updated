...
const OfferCard = ({ offer }: { offer: Offer }) => {
  ...
  // For display purposes: if expired and still actif, show as sous_evaluation
  const displayStatus = isExpired && isActif ? 'sous_evaluation' : offer.status;
      ...
      <div className="p-6 flex-grow">
        <div className="flex justify-between items-start mb-2">
          ...
          <span className={`px-2 py-1 text-xs font-semibold rounded ${
            ...
            displayStatus === 'actif' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
          }`}>
            {hasSelectedCandidate ? t('rh.status.resultat') : 
             displayStatus === 'actif' ? `${t('offer.closes')} ${deadlineDate.toLocaleDateString()}` : 
